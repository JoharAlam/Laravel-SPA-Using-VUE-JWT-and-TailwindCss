<template>
    ...
    <div class="md:h-auto bg-white pl-8 pr-8 pt-4 w-auto rounded-xl shadow-xl">
      ...
      <div id="app" class="container">
        <h6 class="mt-3 text-gray-600">Edit Returned Purchase</h6>
        <hr>
        <div class="row">
          <div class="col pt-3">
          ...
          <div class="col pt-3 text-white">
            ...
            <select id="status" class="border rounded bg-blue-800 w-full pt-4 pl-2 pb-1" style="height:52px;">
              ...
              <option value="Defected Products">Defected Products</option>
        ...
        <br>
          ...
          <div class="col rounded-md">
            <table id="returned-purchases-table" class="display table-bordered nowrap rounded-md bg-blue-500 text-white" cellspacing="0" width="100%">
...
<script>
  ...
  export default {
    ...
    methods: {
      reloadDatatable(){
        let purchases = [];
        this.dataTable = $("#returned-purchases-table").DataTable();

        //To clear datatable rows
        this.dataTable.clear().draw();

        //To reload datatable rows
        const url = "http://vuespajwt.test/return/purchase/detail";
        fetch(url)
        .then((res) => res.json())
        .then((data) => {
          data.forEach((item) => {
          ...
          purchases.forEach((purchase) => {
            this.dataTable.row
              .add([i++, purchase.retailer_name, purchase.product, purchase.category, purchase.quantity, purchase.amount, purchase.status, '<button type="submit" class="pl-2 h-5 w-9 border-2 border-yellow-500 border-opacity-80 rounded hover:bg-yellow-500" onClick="Id('+purchase.id+');Name(\''+purchase.retailer_name+'\');Product(\''+purchase.product+'\');"><svg class="w-4 h-3" viewBox="0 0 18 18" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg></button>'])
            .draw(false);
          });
        });
      },
      updateStatus() {
        ...
        axios.post('http://vuespajwt.test/update/return/purchase', {id, name, product, status}).then((res) =>{
          this.success = '';
          this.errors = [];
      ...
      pay() {
        var id = $("#id").val();
        var name = $("#name").val();
        var receive_able = this.receive_able;

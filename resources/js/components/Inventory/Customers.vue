<template>
    ...
    <div class="md:h-auto bg-white pl-8 pr-8 pt-4 w-auto rounded-xl shadow-xl">
      ...
      <div v-for="error in errors">
        ...
        <h6 class="mt-3 text-gray-600">Edit Customer</h6>
        <hr>
        <div class="row">
          <div class="col pt-3">
            <label for="customer" class="uppercase text-blue-300 text-xs font-bold absolute pl-3 pt-2">Customer ID</label>
          ...
          <div class="col pt-3">
            ...
            <input type="name" id="customer_name" class="pt-4 w-full rounded pl-3 pb-1 bg-blue-800 text-white placeholder-gray-300 outline-none focus:bg-blue-700" placeholder="Customer Name" autocomplete="customer_name" disabled>
          </div>
          <div class="col pt-3">
        ...
        <br>
        <button class="uppercase rounded text-center text-white bg-blue-500 hover:bg-blue-700 py-2 px-3 shadow-xl font-bold" @click="updateCustomer">Update</button>
        <hr>
        <br>
  
        <div class="row rounded-md">
          <div class="col rounded-md">
            <table id="customers-table" class="display table-bordered nowrap rounded-md bg-blue-500 text-white" cellspacing="0" width="100%">
...
export default {
  ...
  methods: {
    reloadDatatable(){
      ...
      const url = "http://vuespajwt.test/customers/detail";
      fetch(url)
      .then((res) => res.json())
      .then((data) => {
        data.forEach((item) => {
        ...
        customers.forEach((customer) => {
          ...
          this.dataTable.row
            .add([i++, customer.customer_name, customer.total, customer.received, customer.receive_able, customer.pay_able, '<button type="submit" class="pl-2 h-5 w-9 border-2 border-yellow-500 border-opacity-80 rounded hover:bg-yellow-500" onClick="editCustomer('+customer.id+');customerName(\''+name+'\');"><svg class="w-4 h-3" viewBox="0 0 18 18" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg></button>'])
            .draw(false);
        });
      });
    },
    updateCustomer() {
      ...
      axios.post('http://vuespajwt.test/update/customer', {id, customer_name, received, paid}).then((res) =>{
        this.success = '';
        this.errors = [];
        if(received == '' && paid == '')
        {
  ...
  mounted() {
    ...
    .then((data) => {
      ...
      customers.forEach((customer) => {
        let name = customer.customer_name;
        this.dataTable.row
          .add([i++, customer.customer_name, customer.total, customer.received, customer.receive_able, customer.pay_able, '<button type="submit" class="pl-2 h-5 w-9 border-2 border-yellow-500 border-opacity-80 rounded hover:bg-yellow-500" onClick="editCustomer('+customer.id+','+customer.total+','+customer.received+','+customer.un_paid+');customerName(\''+name+'\');"><svg class="w-4 h-3" viewBox="0 0 18 18" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg></button>'])
          .draw(false);
      });
    });
  }
